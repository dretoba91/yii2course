    ...
    public function actionApprove_course($id) {
        $query = Registration::find()->where(['id' => $id])->one();
         ...
         if(!empty($query)){
            ...
            $query->save();
            Yii::$app->getSession()->setFlash('success', 'You have approved '.$query->user->name.' Course registeration');
            return $this->redirect('status_update');
         }else{
    ...
    public function actionReject_course($id)
     {
        $query = Registration::find()->where(['id' => $id])->one();
         ...
         if(!empty($query)){
            ...
            $query->save();
            Yii::$app->getSession()->setFlash('success', 'You reject '.$query->user->name.' Course registeration');
            return $this->redirect('status_update');
         }else{
            Yii::$app->getSession()->setFlash('error', 'An error occur');
            return $this->redirect('status_update');
